...
Errors reported by the PostgreSQL server are [`DatabaseError`](/apis/errors) instances. They carry
the server's SQLSTATE `code`, plus `detail`, `constraint`, and the other
[error fields](/apis/errors), which is what you branch on to handle an expected failure such as a
unique violation.
