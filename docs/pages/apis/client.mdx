...
`client.pipeline: boolean`

Whether this client has pipelining enabled. Set its initial value with the `pipeline` config option. Before connecting, it can be changed freely. Once connected, assigning a different value requires the client to be idle and otherwise throws. Defaults to `false`. See [Pipelining](/features/pipelining) for details and examples.
...
`client.isIdle(): boolean`

Returns whether the client is connected, ready for a new query, and has no active, queued, or pipelined queries.

## client.waitForIdle

`client.waitForIdle(): Promise<void>`

Resolves when the client becomes idle. Rejects if the client errors or closes first.
...
try {
  // Run a cursor query and close it.
} finally {
  await client.waitForIdle()
