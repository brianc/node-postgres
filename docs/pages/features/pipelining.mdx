...
You can temporarily disable pipelining for work that needs a non-pipelined connection, such as a cursor. Once connected, the client must be idle before changing modes; assigning a different value to <code>pipeline</code> otherwise throws.
...
try {
  // Run and close the non-pipelined work.
} finally {
  await client.waitForIdle()
...
When the client came from a pool configured with <code>pipeline: true</code>, restore pipelining before calling <code>client.release()</code>.
