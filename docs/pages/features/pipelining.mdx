...
You can temporarily disable pipelining for work that needs a non-pipelined connection, such as a cursor. The client must be idle before changing modes; <code>setPipeline()</code> throws if any query is active, queued, or awaiting a response.
