...
Client.prototype.connect = function(callback) {
  ...
  con.once('end', function() {
    ...
    var query;
    while(query = self.queryQueue.shift()) {
      query.handleError(new Error('Stream unexpectedly ended during query execution'));
    }
    self.emit('end');
  });
...
Client.prototype._pulseQueryQueue = function() {
  if(this._disconnected) {
    var query;
    while(query = this.queryQueue.shift()) {
      query.handleError(new Error('Stream unexpectedly ended during query execution'));
    }
  } else if(this.readyForQuery===true) {
