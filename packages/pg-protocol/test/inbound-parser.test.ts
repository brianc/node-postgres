...
import type { BackendMessage } from '../src/messages.ts'
...
const expectedAuthenticationOkayMessage = { name: 'authenticationOk', length: 8 }
...
const expectedBackendKeyDataMessage = { name: 'backendKeyData', processID: 1, secretKey: 2 }
const expectedReadyForQueryMessage = { name: 'readyForQuery', length: 5, status: 'I' }
const expectedCommandCompleteMessage = { name: 'commandComplete', length: 13, text: 'SELECT 3' }
const emptyRowDescriptionBuffer = new BufferList().addInt16(0).join(true, 'T')
...
const expectedOneRowMessage = {
  ...
  fieldCount: 1,
  fields: [
    { name: 'id', tableID: 1, columnID: 2, dataTypeID: 3, dataTypeSize: 4, dataTypeModifier: 5, format: 'text' },
  ],
}
const expectedTwoRowMessage = {
  ...
  fieldCount: 2,
  fields: [
    { name: 'bang', tableID: 1, columnID: 2, dataTypeID: 3, dataTypeSize: 4, dataTypeModifier: 5, format: 'text' },
    {
      name: 'whoah',
      tableID: 10,
      columnID: 11,
...
const emptyParameterDescriptionBuffer = new BufferList().addInt16(0).join(true, 't')
...
const expectedPlainPasswordMessage = { name: 'authenticationCleartextPassword' }
const expectedMD5PasswordMessage = { name: 'authenticationMD5Password', salt: Buffer.from([1, 2, 3, 4]) }
const expectedSASLMessage = { name: 'authenticationSASL', mechanisms: ['SCRAM-SHA-256'] }
const expectedSASLContinueMessage = { name: 'authenticationSASLContinue', data: 'data' }
const expectedSASLFinalMessage = { name: 'authenticationSASLFinal', data: 'data' }
...
async function parseBuffers(input: Buffer[]): Promise<BackendMessage[]> {
...
function testForMessage(buffer: Buffer, expectedMessage: any) {
...
describe('PgPacketStream', () => {
  ...
  testForMessage(buffers.emptyQuery(), { name: 'emptyQuery', length: 4 })
  testForMessage(Buffer.from([0x6e, 0, 0, 0, 4]), { name: 'noData' })
  ...
  describe('parsing rows', () => {
    describe('parsing empty row', () => {
      testForMessage(emptyRowFieldBuf, { name: 'dataRow', fieldCount: 0 })
    })

    describe('parsing data row with fields', () => {
      testForMessage(oneFieldBuf, { name: 'dataRow', fieldCount: 1, fields: ['test'] })
    })
  })

  describe('notice message', () => {
    const buff = buffers.notice([{ type: 'C', value: 'code' }])
    testForMessage(buff, { name: 'notice', code: 'code' })
  })

  testForMessage(buffers.error([]), { name: 'error' })
  ...
  testForMessage(parseCompleteBuffer, { name: 'parseComplete' })
  testForMessage(bindCompleteBuffer, { name: 'bindComplete' })
  testForMessage(buffers.closeComplete(), { name: 'closeComplete' })

  describe('parses portal suspended message', () => {
    testForMessage(portalSuspendedBuffer, { name: 'portalSuspended' })
  })

  describe('parses replication start message', () => {
    testForMessage(Buffer.from([0x57, 0x00, 0x00, 0x00, 0x04]), { name: 'replicationStart', length: 4 })
  })

  describe('copy', () => {
    testForMessage(buffers.copyIn(0), { name: 'copyInResponse', length: 7, binary: false, columnTypes: [] })
    testForMessage(buffers.copyIn(2), { name: 'copyInResponse', length: 11, binary: false, columnTypes: [0, 1] })
    testForMessage(buffers.copyOut(0), { name: 'copyOutResponse', length: 7, binary: false, columnTypes: [] })
    testForMessage(buffers.copyOut(3), { name: 'copyOutResponse', length: 13, binary: false, columnTypes: [0, 1, 2] })
    testForMessage(buffers.copyDone(), { name: 'copyDone', length: 4 })
  ...
  describe('split buffer, single message parsing', () => {
    ...
    const testMessageReceivedAfterSplitAt = async (split: number) => {
    ...
    it('parses when split in the middle', () => testMessageReceivedAfterSplitAt(6))
    it('parses when split at end', () => testMessageReceivedAfterSplitAt(2))
  ...
  describe('split buffer, multiple message parsing', () => {
    ...
    const readyForQueryBuf = buffers.readyForQuery()
    const fullBuffer = Buffer.alloc(dataRowBuffer.length + readyForQueryBuf.length)
    dataRowBuffer.copy(fullBuffer, 0, 0)
    readyForQueryBuf.copy(fullBuffer, dataRowBuffer.length, 0)

    const verifyMessages = (messages: any[]) => {
      expect(messages.length).toBe(2)
      expect(messages[0]).toEqual({ name: 'dataRow', fieldCount: 1, length: 11, fields: ['!'] })
      expect(messages[0].fields[0]).toBe('!')
      expect(messages[1]).toEqual({ name: 'readyForQuery', length: 5, status: 'I' })
    }

    it('receives both messages when packet is not split', async () => {
      const messages = await parseBuffers([fullBuffer])
    ...
    describe('receives both messages when packet is split', () => {
      it('in the middle', () => splitAndVerifyTwoMessages(11))
      it('at the front', () =>
        Promise.all([
      ...
      it('at the end', () => Promise.all([splitAndVerifyTwoMessages(8), splitAndVerifyTwoMessages(1)]))
    })
  })

  it('cleans up the reader after handling a packet', () => {
    const parser = new Parser()
    parser.parse(oneFieldBuf, () => {})
    expect((parser as any).reader.buffer.byteLength).toBe(0)
  })
})
