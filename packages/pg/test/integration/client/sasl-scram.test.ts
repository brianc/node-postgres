...
describe('sasl-scram', () => {
  ...
  if (native) {
    it('skipping SCRAM tests (on native)', () => {})
    return
  }
  if (!config.user || !config.password) {
    it('skipping SCRAM tests (missing env)', () => {})
    return
  }

  it('can connect using sasl/scram with channel binding enabled (if using SSL)', async () => {
  ...
  it('can connect using sasl/scram with channel binding disabled', async () => {
  ...
  it('sasl/scram fails when password is wrong', async () => {
  ...
  it('sasl/scram fails when password is empty', async () => {
    const client = new pg.Client({
      ...config,
