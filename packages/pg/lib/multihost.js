...
function probeType(targetAttrs) {
  switch (targetAttrs) {
    case 'read-write':
    case 'read-only':
      return 'tx_read_only'
...
function applyProbeResult(probeType, row, params) {
...
function canDecideFromParams(targetAttrs, params) {
  switch (targetAttrs) {
    case 'read-write':
    case 'read-only':
...
function hostMatches(targetAttrs, params, hostIndex, hostCount, preferStandbyPass) {
  switch (targetAttrs) {
    case 'read-write':
