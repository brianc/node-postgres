...
const params = {
  ...
  values: [-1, true] as unknown[],
}

const insert = {
  text: 'INSERT INTO foobar(name, age) VALUES ($1, $2)',
  values: ['brian', 100] as unknown[],
}
...
const exec = async (client: Client, q: Q): Promise<unknown> => {
  return client.query({
    text: q.text,
    values: q.values,
    rowMode: 'array',
...
const bench = async (client: Client, q: Q, time: number): Promise<number> => {
...
const run = async (): Promise<void> => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const start = performance.now()
    const results = (await client.query('SELECT * FROM buf')) as { rows: Array<{ data: ArrayBufferLike }> }
