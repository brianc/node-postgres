...
pg.connect(helper.config, function(err, client, done) {
  if(err !== null) {
    console.error("Received connection error when attempting to contact PostgreSQL:");
    console.error(err);
    process.exit(255);
  }
  ...
  client.query("SELECT COUNT(name) FROM person", function(err, callback) {
    if(err != null) {
      console.error("Received error when executing query 'SELECT COUNT(name) FROM person'");
      console.error("It is possible you have not yet run the table create script under script/create-test-tables.js");
